...
from operation.contract.client.collection import collection_add
from test_cases.contract.client.conftest import *
from common.logger import logger


class Test_collection_add:
    '''
        收藏交易对:
            1, 查询当前账户 accountId
    ...
    @pytest.mark.parametrize("scene,symbol,except_result,except_code,except_msg",
                             api_collection_add["test_collection_add"])
    def test_collection_add(self,scene,symbol,except_result,except_code,except_msg):
        # logger.info("*************** 开始执行用例 ***************")
        logger.info(f'场景【{scene}】信息：{symbol}-{except_result}-{except_code}-"{except_msg}"')
        result = collection_add.collection_add(symbol)
        ...
        try:
            if result.response.get("msgInfo") !="failure":
                ...
                # 判断当前交易对是否在收藏列表之中
                assert symbol in collection_all
...
if __name__ == '__main__':
    pytest.main(["-q", "-s", "Test_collection_add.py"])
