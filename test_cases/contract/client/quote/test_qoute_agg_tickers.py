...
'''
from operation.contract.client.quote.qoute_agg_tickers import qoute_agg_tickers
from test_cases.contract.client.conftest import *
from common.logger import logger


class Test_qoute_agg_tickers:
    '''
        获取指定交易对的聚合行情信息:
    '''
    ...
    @pytest.mark.parametrize("scene,except_result,except_code,except_msg",
                             api_quote_data["qoute_agg_tickers"])
    def test_qoute_agg_tickers(self,scene,except_result,except_code,except_msg):
        # logger.info("*************** 开始执行用例 ***************")
        logger.info(f'场景【{scene}】信息：{except_result}-{except_code}-"{except_msg}"')
        result = qoute_agg_tickers()
        assert except_result == result.response["msgInfo"]
        assert except_code == result.response["returnCode"]
        if except_code == 0:
            assert except_msg in str(result.response["msgInfo"])
        else:
